...
# Cache decorator
def cache_response(expire_time: int = 300):  # Default 5 minutes
    def decorator(func):
        @wraps(func)
        async def wrapper(*args, **kwargs):
            ...
            logger.info(f"Cache miss for {cache_key}, stored in cache")
...
@cache_response(expire_time=int(os.getenv('SALES_CACHE_DURATION', 300)))
async def get_data(
...
@cache_response(expire_time=int(os.getenv('AI_CACHE_DURATION', 600)))
